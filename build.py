...
ver_str = "0.1.0-deltakind"
windows_ver = "0, 1, 0, 0"
file_description = "DeltaKind - Deltarune fangame"
is_standalone = True # DeltaKind standalone build
...
# Standalone release must not contain development snapshots inside projects.
deltakind_snapshots = os.path.join(
    build_path,
    "kristal",
    "mods",
    "deltakind",
...
if os.path.isdir(deltakind_snapshots):
    shutil.rmtree(deltakind_snapshots)
    print("Removed DeltaKind development snapshots from release staging.")

shutil.make_archive(
    os.path.join(build_path, "kristal"),
    'zip',
    os.path.join(build_path, "kristal")
)
...
setInfo("InternalName", "DeltaKind")
setInfo("LegalCopyright", "Copyright © 2026 DeltaKind")
setInfo("OriginalFilename", "DeltaKind.exe")
setInfo("ProductName", "DeltaKind")
...
shutil.make_archive(os.path.join(output_path, "deltakind-"+ver_str+"-love"), 'zip', os.path.join(build_path, "lovepkg"))
shutil.make_archive(os.path.join(output_path, "deltakind-"+ver_str+"-win"), 'zip', os.path.join(build_path, "executable"))

print("Standalone DeltaKind build: skipping example project packaging.")

print("Done!")
print("Generated files:")
print("> deltakind-"+ver_str+"-love.zip")
print("> deltakind-"+ver_str+"-win.zip")
