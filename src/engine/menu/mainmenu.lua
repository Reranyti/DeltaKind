...
function MainMenu:enter()
    ...
    self.meta_save = MainMenuMetaSave(self)
    self.achievements = MainMenuAchievements(self)
    self.archive_dialogue = MainMenuArchiveDialogue(self)
    ...
    self.state_manager:addState("METASAVE", self.meta_save)
    self.state_manager:addState("ACHIEVEMENTS", self.achievements)
    self.state_manager:addState("ARCHIVEDIALOGUE", self.archive_dialogue)
...
function MainMenu:draw()
    ...
    -- Side-B menu theme.
    -- A blood-red overlay is drawn before the stage,
    -- so the blue soul remains visible in its own color.
    if Kristal.Config.sideB then
        Draw.setColor(0.65, 0.02, 0.02, 0.55)
