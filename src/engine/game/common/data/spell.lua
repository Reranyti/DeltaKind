...
function Spell:getTPCost(chara)
    ...
    if max_tension == 100 then
        return self.cost
    end

    return math.floor((self.cost / 100) * max_tension + 0.5)
end
...
function Spell:getPowerMenuTPDisplay(chara)
    local cost = self:getTPCost(chara)
    local max_tension = Game:getMaxTension()

    if max_tension <= 0 then
        return "0%"
    end

    return tostring(math.floor((cost / max_tension) * 100 + 0.5)) .. "%"
end
