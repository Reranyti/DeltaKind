...
local max_page = math.ceil(#Game.battle.menu_items / 6) - 1
    ...
    Draw.draw(
        self.heart_sprite,
        5 + ((Game.battle.current_menu_x - 1) * 230),
        30 + ((Game.battle.current_menu_y - (page * 3)) * 30)
    )

    local MENU_FONT_SCALE = 1
    local font = Assets.getFont("deltarune-cyrillic1", 16)
    ...
    for i = page_offset + 1, math.min(page_offset + 6, #Game.battle.menu_items) do
        ...
        if item.party then
            ...
            for index, party_id in ipairs(item.party) do
                ...
                if Game.battle:getPartyIndex(party_id) ~= Game.battle.current_selecting then
                    ...
                    Draw.draw(
                        Assets.getTexture(chara:getHeadIcons() .. "/head"),
                        text_offset + 30 + (x * 230) + ox,
                        50 + (y * 30) + oy
                    )
        ...
        local item_x = text_offset + 30 + (x * 230)
        local item_y = 50 + (y * 30)
        ...
        text_offset = text_offset + (font:getWidth(item.name) * MENU_FONT_SCALE)
        ...
        if item.icons then
            ...
            for _, icon in ipairs(item.icons) do
                ...
                if not icon[2] then
                    ...
                    Draw.draw(
                        texture,
                        text_offset + 30 + (x * 230) + (icon[3] or 0),
                        50 + (y * 30) + (icon[4] or 0)
                    )
    ...
    if current_item then
        ...
        if current_item.tp and current_item.tp ~= 0 then
            Draw.setColor(
                Game.battle and Game.battle:hasReducedTension()
                    and PALETTE["tension_desc_reduced"]
                    or PALETTE["tension_desc"]
            )

            love.graphics.print(
                math.floor((current_item.tp / Game:getMaxTension()) * 100) ..
                "% " ..
                Game:getConfig("tpName"),
                260 + 240,
                50 + (tp_offset * 32)
            )
...
elseif Game.battle.state == "ENEMYSELECT" then
    ...
    for index = page_offset + 1, math.min(page_offset + 3, #enemies) do
        ...
        if enemy then
            ...
            for i = 1, #enemy.icons do
                if enemy.icons[i] then
                    if (spare_icon and (i == 1)) or (tired_icon and (i == 2)) then
                        ...
                        Draw.draw(
                            enemy.icons[i],
                            80 + font:getWidth(enemy.name) + (i * 20),
                            60 + y_off
                        )
                    end
                end
            ...
            if Game.battle.state_reason == "XACT" then
                Draw.setColor(
                    Game.battle.party[Game.battle.current_selecting].chara:getXActColor()
                )

                if Game.battle.selected_xaction.id == 0 then
                    love.graphics.print(
                        enemy:getXAction(
                            Game.battle.party[Game.battle.current_selecting]
                        ),
                        self.xact_x_pos,
                        50 + y_off
                    )
                    ...
                    love.graphics.print(
                        Game.battle.selected_xaction.name,
                        self.xact_x_pos,
                        50 + y_off
                    )
                end
...
elseif Game.battle.state == "PARTYSELECT" then
    ...
    for index = page_offset + 1, math.min(page_offset + 3, #Game.battle.party) do
        ...
        love.graphics.print(
            Game.battle.party[index].chara:getName(),
            80,
            50 + ((index - page_offset - 1) * 30)
        )
        ...
        love.graphics.rectangle(
            "fill",
            400,
            55 + ((index - page_offset - 1) * 30),
            101,
            16
        )

        local percentage =
            Game.battle.party[index].chara:getHealth()
...
if Game.battle.state == "ATTACKING" or self.attacking then
    ...
    if not Game:getConfig("oldUIPositions") then
        ...
        love.graphics.rectangle(
            "fill",
            has_index[2] and 77 or 2,
            78,
            has_index[2] and 226 or 301,
            3
        )

        love.graphics.rectangle(
            "fill",
            has_index[3] and 77 or 2,
            116,
            has_index[3] and 226 or 301,
            3
        )
    end
end
