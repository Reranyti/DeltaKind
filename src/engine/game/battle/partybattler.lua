...
function PartyBattler:hurt(amount, exact, color, options)
    ...
    if (self.chara:getHealth() <= 0) then
        ...
        -- ПРИМЕЧАНИЕ: раньше здесь тоже стоял 4-й аргумент
        -- (kill=true), из-за чего КАЖДОЕ попадание убивало
        -- ВСЕ другие числа урона во всём бою (см. DamageNumber:
        -- onAdd — kill_others не различает, к какому именно
        -- бойцу относится число, он проходит по всем детям
        -- Battle целиком). При нескольких попаданиях по разным
        -- членам партии почти одновременно (обычное дело в
        -- волнах вроде Fracture/Hazard) это стирало ещё не
        -- доигравшие числа урона — именно баг "урон иногда
        -- исчезает во время атак". У EnemyBattler для обычного
        -- урона kill вообще не передаётся (см. enemybattler.lua,
        -- строка ~798) — здесь убираем его по той же логике,
        -- оставляя kill=true только для "down"/"swoon" выше,
        -- где стирание старых чисел действительно осмысленно.
        self:statusMessage("damage", amount, color)
    end
